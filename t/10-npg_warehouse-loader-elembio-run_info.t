...
subtest 'construct object, test for errors when loading' => sub {
  ...
  is ($loader->run_folder, 'some', 'run folder name, path does not exist');
...
subtest 'load the data' => sub {
  ...
  is ($row->run_stats, slurp($stats_file),
    'run statistics file is saved correctly');
  ...
  is ($row->run_stats, undef, 'run statustics has not been loaded');
  ...
  cp $stats_file, "$new_dir/AvitiRunStats.json"; # step 2
  
  $loader = npg_warehouse::loader::elembio::run_info->new(
    schema_wh => $schema_wh, runfolder_path => $new_dir
  );
  is ($loader->load(), 1, 'loading worked');
  is ($schema_wh->resultset('EseqRun')->count(), 2, 'eseq_run table has 2 rows');
  $row = $rs->search({folder_name => $rf_name})->next();
  ok ($row->run_stats, 'run stats file content is loaded');
  is ($row->date_completed, undef, 'run completion date is undefined');
  is ($row->outcome, undef, 'run outcome is undefined');

  # copy a file, preserve timestamp
  `cp -p $path/RunUploaded.json $new_dir/RunUploaded.json`; # step 3
  $loader = npg_warehouse::loader::elembio::run_info->new(
    schema_wh => $schema_wh, runfolder_path => $new_dir
  );  
  is ($loader->load(), 1, 'loading worked');
  ...
  is ($row->date_completed, '2025-02-13T21:21:42', 'run completion date is correct');
