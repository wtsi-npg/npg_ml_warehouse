...

{
  ...
  is($r->command, "$command --loop --sleep-time 1200", 'command to run');
  ...
  my $action = $test . qq[daemon -i -r -a 10 -n $command --umask 002 -A 10 -L 10 -M 10 -o $log_dir/$command-$host-2013.log -- $command --loop --sleep-time 1200] . $error;
